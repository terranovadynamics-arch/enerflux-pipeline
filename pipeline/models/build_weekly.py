...
def build_weekly(wti_csv="pipeline/outputs/WTI_DAILY_latest.csv"):
    ...
    # Lire la série quotidienne (présente grâce à ci_run.py weekly)
    if not Path(wti_csv).exists():
        raise FileNotFoundError(f"{wti_csv} absent. Exécute d'abord la collecte quotidienne.")

    df = pd.read_csv(wti_csv, parse_dates=["date"]).dropna()
    df = df.set_index("date").resample("W").mean().reset_index()
    if df.empty:
        raise RuntimeError("Série vide après resample hebdomadaire.")
    ...
    tbl = plt.table(
        cellText=df.tail(12).round(2).values,
...
if __name__ == "__main__":
    ...
    # Push R2 uniquement si secrets présents
